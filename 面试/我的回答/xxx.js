...
有一个场景，10个组件都调用了同一个接口，那么怎么办，不更改组件内在的逻辑，如何让请求只发一次？（我后面说在重新封装axios，然后我提到了拦截URL，后面经过面试官提醒想到了请求拦截器）
    ...
    http版本有了解哪些吗？（http1.01.12.03.0，队头阻塞，服务端推送，头部压缩，range断点续传，cachecontrol等等）
...
8. 有了解过react吗
9. vue和react的区别
3、js的事件循环机制4、定时器是宏任务还是微任务，定时1分钟会精确执行吗？5、vue路由懒加载6、如何设计一个vue搜索框，具有模糊搜索功能7、watch和computed9、手写：找出数组中重复的数字10、手写：数组中第K大的数字
...
clearTimeout / clearInterval 能否相互清除对方的定时器的问题。这个我答的不会，但结果是会的...
...
react释放不了闭包，具体为什么
箭头函数
 一道牛客上的算法，对应力扣86题，牛客上的描述极坑，问了面试官才看懂题目，A了
盒子模型
封装过哪些组件
登陆注册流程
算法部分是自己做的吗（inSight）
