### 从输入 URL 到页面展示，这中间发生了什么？包括之前说的缓存的情况
...
### 跨域的响应头，假设我现在只想要 get、post 这两个请求跨域，应该如何设置

跨域资源共享（Cross-Origin Resource Sharing，简称 CORS）允许在一个域名下的网页向另一个域名的服务器发起 XMLHttpRequest 请求，属于跨域通信的一种常用解决方案。为了确保安全性和控制访问权限，需要在服务器端设置相应的响应头。

以下是一些常见的跨域响应头选项：

1. Access-Control-Allow-Origin：指定哪些源（origin）可以访问资源，可以使用通配符 "\*" 表示允许任意来源访问。
2. Access-Control-Allow-Methods：指定允许的 HTTP 方法，例如 GET、POST、PUT、DELETE 等。
3. Access-Control-Allow-Headers：指定允许的自定义请求头，如 Content-Type、Authorization 等。
4. Access-Control-Expose-Headers：指定允许客户端访问的响应头，除了默认可访问的头部信息（如 Cache-Control、Content-Language 等）之外的其他头部信息需要在这里明确列出。
5. Access-Control-Allow-Credentials：指定是否允许发送身份凭证（如 cookies）到服务器。
6. Access-Control-Max-Age：指定预检请求（OPTIONS）的缓存时间，单位为秒。在此时间内，浏览器会直接使用该响应而不再发送预检请求。
...
上述示例中，设置了允许 https://www.example.com 这个源发起跨域请求，仅允许使用 GET 和 POST 方法，以及允许的请求头包括 Content-Type 和 Authorization。同时，设置允许客户端访问 X-Custom-Header 响应头，允许发送身份凭证，并将预检请求的缓存时间设为 86400 秒。

请注意，具体需要设置哪些响应头选项，以及其取值，应根据具体的需求和安全考虑而定。

**如果你只想允许 GET 和 POST 请求进行跨域访问，可以在服务端设置如下响应头：**
...
上述设置中，`Access-Control-Allow-Origin` 设置为 `*` 表示允许任意源（origin）进行跨域访问，而 `Access-Control-Allow-Methods` 则指定了允许的 HTTP 方法为 GET 和 POST。

请注意，使用通配符 `*` 是一种简便的方式来允许任意源进行跨域访问，但这也会带来安全风险。如果你知道请求会来自哪些具体的源，推荐将 `Access-Control-Allow-Origin` 设置为相应的源，例如：

```
Access-Control-Allow-Origin: https://www.example.com
```

这样只有来自 `https://www.example.com` 的请求才能跨域访问。

此外，还可以添加其他的跨域相关响应头，例如 `Access-Control-Allow-Headers` 来指定允许的自定义请求头，以及 `Access-Control-Allow-Credentials` 来允许发送身份凭证（如 cookies）到服务器等。根据具体的需求和安全考虑，可以选择适当的响应头进行设置。

![alt](https://uploadfiles.nowcoder.com/images/20230606/439072644_1686043003227/AD1A1B7183951DC89F508A6B6C422144)

![1687522075029](image/计算机基础&&浏览器/1687522075029.png)

![1687522065406](image/计算机基础&&浏览器/1687522065406.png)

[https://www.bilibili.com/video/BV1a34y167AZ?p=54&amp;vd_source=79123c85f3fc73dc574307550dbc4600](https://www.bilibili.com/video/BV1a34y167AZ?p=54&vd_source=79123c85f3fc73dc574307550dbc4600)
...
### HTTP 请求中 token、cookie、session 有什么区别，Cookie 和 Session 的作用,session 的工作原理
...
将身份验证令牌（Token）存储在 Cookie 或 LocalStorage 中有以下几个原因：

1. 方便的客户端存储：Cookie 和 LocalStorage 是浏览器提供的客户端存储机制，可以将数据以键值对的形式保存在用户的浏览器中。这样做可以方便地在后续的请求中使用 Token，而无需频繁地将 Token 作为参数传递。
2. 持久化存储：Cookie 和 LocalStorage 都具有持久性，即使用户关闭浏览器再次打开，存储在其中的 Token 仍然存在。这使得用户在一段时间内保持登录状态成为可能，提供更好的用户体验。
3. 跨页面访问：Cookie 和 LocalStorage 存储在浏览器中，可以在不同页面之间共享和访问。这使得在整个网站范围内，用户的 Token 都可以被识别和使用，无需重复登录或重新授权。
4. 跨域支持：由于 Cookie 和 LocalStorage 是基于客户端的存储机制，它们可以轻松地在不同域名或跨域的情况下使用。而其他存储方式，如 Session，通常会受到同源策略的限制。

请注意，使用 Cookie 或 LocalStorage 存储 Token 也需要注意安全性。为了防止 Token 泄露或被盗用，应该采用安全措施，如对 Cookie 设置 HttpOnly 和 Secure 标志来防止 XSS 攻击，以及合理限制 Token 的过期时间等。
...
### JWT 是干什么的?还有什么其他方式能实现 JWT 的类似功能吗?，JWT 中使用到了什么加密算法？介绍一下 SHA256 加密过程?JWT 的优缺点？

JWT (JSON Web Token) 是一种开放标准，用于在网络应用程序和服务之间安全地传输信息。它通常用于验证用户身份和授权访问。

除了 JWT，还有类似功能的技术，如基于会话的认证和基于令牌的认证（例如 OAuth 和 OpenID Connect）。但是，JWT 比这些技术更轻量级，更易于使用，并且不需要在服务器端保留任何状态。

JWT 使用的加密算法包括 HMAC、RSA 和 ECDSA。其中，SHA256 是一种哈希函数，它将输入数据转换为固定长度的唯一输出。SHA256 使用的过程包括初始化、消息填充、处理数据块和输出结果。

JWT 的优点包括易于使用、轻量级、跨语言支持和可扩展性。缺点包括无法撤销令牌、需要额外处理重要数据保护等方面的安全性考虑、以及对客户端存储的依赖。

[jwt](https://www.bilibili.com/video/BV1b14y1J7Yv/?spm_id_from=333.788&vd_source=79123c85f3fc73dc574307550dbc4600)
...
1. https://a.tencent.com/foo/bar
2. http://a.tencent.com/foo/bar
3. https://a.tencent.com/foo/aaa
4. https://b.tencent.com/foo/bar

### 结合 `cookie`实现广告在某个时间段投放，到时就过期

可以通过在 cookie 中设置一个过期时间来实现广告在某个时间段投放，并在到时就过期的效果。

具体实现步骤如下：

1. 在需要投放广告的页面中，通过 JavaScript 代码设置一个带有过期时间的 cookie。例如：

```
var now = new Date();
var expireTime = now.getTime() + 60 * 60 * 1000; // 设置过期时间为当前时间加上一个小时
now.setTime(expireTime);
document.cookie = "adShown=true; expires=" + now.toGMTString();
```

2. 在广告代码中，先判断该 cookie 是否已经过期，如果过期则不进行投放。例如：缓存
...
这样，在每次加载页面时，都会检查 cookie 中的过期时间，如果过期了就不再进行广告投放。同时，设置过期时间为当前时间加上一定的时间，从而实现广告在某个时间段投放，并在到时就过期的效果。

需要注意的是，由于 cookie 存储在客户端，因此无法保证绝对的安全性。如果需要更高的安全性，可以考虑使用服务端的 Session 或 JWT 等技术进行实现。
...
### localhost 和 127.0.0.1 的区别有哪些

本地应用建议使用 localhost。

localhost 与 127.0.0.1 的区别：

localhost 的意思是本地服务器，127.0.0.1 是本机地址，他们的关系是通过操作系统中的 hosts 文件，将 localhost 解析为 127.0.0.1。

在实际工作中，localhost 是不经过网卡传输的，所以，它不受网络防火墙和网卡相关的种种限制；而 127.0.0.1 则是要经过网卡传输数据的，是必须依赖网卡的。这一点是它的最大的区别。

一般设置程序的本地应用时，本地服务用 localhost 是最好的，它不会解析成 IP，也不会占用网卡、网络资源。有时候用 localhost 可以，但用 127.0.0.1 却不行的情况就在于此。

[【计算机网络】一文带你看懂 localhost、127.0.0.1、本机 IP、公有 IP、私有 IP、网卡 (MAC) 与 IP.. - 掘金](https://juejin.cn/post/7013538093974323231)

[2-5 _127.0.0.1_ &amp;_localhost_](https://www.bilibili.com/video/av843334766/?p=7)
...
[一、性能优化方法论 | 前端进阶之旅](https://interview.poetries.top/principle-docs/comprehensive/13-%E6%89%93%E9%80%A0%E5%89%8D%E7%AB%AF%E7%9B%91%E6%8E%A7%E7%B3%BB%E7%BB%9F.html#%E5%85%AB%E3%80%81%E4%BC%98%E5%8C%96%E6%89%8B%E6%AE%B5-%E7%99%BD%E5%B1%8F-300ms-%E5%92%8C%E7%95%8C%E9%9D%A2%E6%B5%81%E7%95%85%E4%BC%98%E5%8C%96%E6%8A%80%E5%B7%A7)
...
SEO（Search Engine Optimization）是指通过优化网站和内容，以提高网站在搜索引擎结果页（SERP）中的排名，从而增加有机流量和网站的可见性。在 SEO 中，存在着两种不同的方法论：黑帽 SEO 和白帽 SEO。

1. 黑帽 SEO：黑帽 SEO 是指使用违反搜索引擎规则和伦理准则的技术手段来提高排名和流量的做法。这些技术手段可能会对搜索引擎的算法进行欺骗，包括但不限于关键词堆砌、隐藏文本、虚假链接、网站克隆等。虽然黑帽 SEO 可能在短期内获得一些成果，但一旦被搜索引擎发现，网站可能会受到惩罚，包括降低排名、移除索引等，从而对网站造成长期不利影响。
2. 白帽 SEO：白帽 SEO 是指遵守搜索引擎规则和伦理准则，通过正当手段优化网站和内容的做法。白帽 SEO 注重用户体验和内容质量，通过优化网站结构、关键词研究、原创内容、内部链接、外部链接建设等方法，提高网站的质量和价值，使其在搜索引擎中获得更好的排名和流量。白帽 SEO 的成果虽然可能需要较长时间，但是对网站的可持续发展和品牌声誉有着积极的影响。

总结来说，黑帽 SEO 采用违反规则和伦理准则的技术手段，可能会带来短期成果，但风险较大；而白帽 SEO 注重合法、道德的优化方法，虽然效果可能需要时间，但对网站的长期发展更加有益。因此，建议选择白帽 SEO 来提升网站的可见性和流量，树立良好的品牌形象，并避免黑帽 SEO 带来的潜在风险。
...
https://zhuanlan.zhihu.com/p/569905141

https://www.bilibili.com/video/BV1tL4y1T7z6/?vd_source=79123c85f3fc73dc574307550dbc4600

### 重绘重排区别 回流和重绘的概念和情况

[基础篇 | 前端进阶之旅](https://interview.poetries.top/docs/base.html#_48-%E9%87%8D%E7%BB%98%E5%92%8C%E5%9B%9E%E6%B5%81-%E9%87%8D%E6%8E%92-%E6%98%AF%E4%BB%80%E4%B9%88-%E5%A6%82%E4%BD%95%E9%81%BF%E5%85%8D)
