...
- 「**500 Internal Server Error**」与 400 类型，是个笼统通用的错误码，服务器发生了什么错误，我们并不知道。状态码500代表服务端内部错误（Internal Server Error），指的是服务器在处理请求时发生了意料之外的错误，导致无法正常响应客户端的请求。这种错误通常是由于代码编写错误、服务器配置问题、数据库连接问题或者其他未知错误引起的。当客户端请求服务器时，如果服务器返回状态码500，说明服务器在处理请求时出现了错误，无法正常响应客户端的请求。客户端可以根据这个状态码来判断请求是否成功，如果收到状态码500，就需要重新尝试请求或者向服务器管理员寻求帮助。
  
  通常情况下，服务器会在响应状态码500的同时，在响应正文中包含一些详细的错误信息，以便开发人员或管理员快速定位和解决问题。但是需要注意的是，这些错误信息对于攻击者来说也是有利可图的，因此在生产环境中要避免将具体的错误信息暴露给客户端。
...
[🐲【2】ShutdownHTTP系列-HTTP报文篇 - 掘金](https://juejin.cn/post/6844904168549777422#heading-9)
