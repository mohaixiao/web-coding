...
要[阻止事件冒泡](https://so.csdn.net/so/search?q=%E9%98%BB%E6%AD%A2%E4%BA%8B%E4%BB%B6%E5%86%92%E6%B3%A1&spm=1001.2101.3001.7020)很简单，两个方法：1、调用 **event.stopPropagation()** 方法就行了，其中 event 就是事件参数；2、**return false**
...
### 如何将对象转为 JSON？如何让这个 JSON 格式化？

[37.JavaScript 对象与 JSON 格式的转换，JSON.stringify、JSON.parse 方法的使用方法和注意事项\_@魏大大的博客-CSDN 博客](https://blog.csdn.net/weixin_43302112/article/details/125468973)

https://www.zhihu.com/question/591067267/answer/2947176114?utm_id=0
...
![1687671327620](image/js/1687671327620.png)

https://www.yuque.com/cuggz/interview/vgbphi?inner=xdqXW
...
在 JavaScript 中，扩展运算符（也称为展开运算符）是一个三个点 `...`，它可以将一个可迭代对象（例如数组、字符串或类数组对象等）展开成一个逗号分隔的列表，或者把多个参数合并成一个数组。

其作用主要有以下几种：

1. 展开数组：可以将一个数组展开成独立的元素，并使用这些元素创建新的数组或将它们传递给函数作为参数。例如：
...
2. 拷贝对象：可以使用扩展运算符浅拷贝一个对象，从而创建一个新的对象，并可以修改其中的属性。例如：

```
const obj1 = { name: 'Alice', age: 20 };
const obj2 = { ...obj1, age: 21 }; // { name: 'Alice', age: 21 }
```

3. 创建副本：可以使用扩展运算符创建一个原数组的副本，并进行修改，而不影响原数组。例如：
...
4. 合并对象：可以使用扩展运算符将多个对象合并成一个对象。如果合并的对象中存在相同的属性，则后面的属性值会覆盖前面的属性值。例如：

```
const obj1 = { name: 'Alice', age: 20 };
const obj2 = { gender: 'female', age: 21 };
const obj3 = { ...obj1, ...obj2 }; // { name: 'Alice', age: 21, gender: 'female' }
```
